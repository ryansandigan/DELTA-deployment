...
!!! warning "This page assumes the deployment is already in service"

    Deployed and bootstrapped through the
    [installation guide](index.md#install-delta), with the built-in
    ...
    **Do not use `docker compose up -d` to bring up a deployment for the first
    ...
    through the [installation guide](index.md#install-delta), which starts the
    application privately, fixes that, and publishes afterwards.
...
> migration that cannot be undone. Use [Update](update.md).
...
!!! danger "`-v` deletes the named volumes"

    **The entire database, every uploaded file, and the logs. It cannot be
    undone, and Docker does not ask for confirmation.**
...
| Update DELTA | [Update](update.md) |
| Back up | [Backup and Restore](backup-restore.md) |
