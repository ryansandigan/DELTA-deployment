...
- [Windows](docker/windows.md) — Windows 10/11 and Windows Server
  2022/2025, with Docker Desktop
- [macOS](docker/macos.md) — Docker Desktop
- [Ubuntu](docker/ubuntu.md) · [Debian](docker/debian.md) ·
  [AlmaLinux](docker/almalinux.md) — Docker Engine
...
> [Deployment Options](deployment-options.md) **before** you start.
...
| `POSTGRES_PASSWORD` | The generated database password. **External database:** set `DATABASE_URL` instead, per [Deployment Options](deployment-options.md#b-external-database) |
...
Skip this step only for `http://localhost` testing.

!!! warning "A real hostname needs HTTPS before anyone uses it"

    DELTA marks session cookies `Secure`, so at a real hostname over plain HTTP
    users sign in and are immediately signed out again.

**Using your own gateway?** It terminates TLS — skip this step, but meet the
requirements in
[Deployment Options](deployment-options.md#what-your-gateway-must-do).
...
!!! danger "Do not publish the site yet"

    Do not run `docker compose up -d` at this point — that starts nginx and
    publishes the site while the built-in administrator account still has the
    password shipped in the public image. Using your own gateway? Leave its
    route disabled.
...
!!! warning "A healthy container is not proof the schema loaded"

    The schema step does not stop on error, so it can fail partway while the
    container still reports healthy. Read the log and query the schema version.
...
Then read the schema version back:
...
=== "External database"
    ...
    This form takes the connection string from the container:
...
[Troubleshooting](troubleshooting.md#startup-and-migrations).
...
> [Deployment Options](deployment-options.md#replacing-the-administrator-password-on-an-external-database),
...
=== "Bash/zsh"
    ...
    This form works in bash, zsh and dash alike. If you interrupt it part-way,
    run `stty echo` to make typing visible again:
...
This uses the same check the application uses:
...
!!! danger "If it does not print `t`, stop"

    The password was not replaced. Do not continue to step 7 and do not make
    this deployment reachable.
...
!!! warning "Using your own gateway?"

    There is no bundled nginx to configure, start or check — skip to
    *Allowing network access* below, publish the `delta` backend port as
    described in
    [Deployment Options](deployment-options.md#c-existing-proxy-load-balancer-or-waf),
    and enable the gateway route now. Not before step 6.3 passed: an
    unprotected backend is reachable whether or not the gateway route exists.
...
[Deployment Options](deployment-options.md#choosing-the-published-address-this-is-a-security-decision).
...
dropped — [Troubleshooting](troubleshooting.md#https-and-sessions). An
HTTP 413 on upload means a proxy is refusing the body size —
[Troubleshooting](troubleshooting.md#http-413-on-upload).

**Before handing this over:** pin `DELTA_IMAGE` to a digest
([Update](update.md#pin-the-image)), and take and test a backup
...
changes — is in [Lifecycle](lifecycle.md), which also points to
[Update](update.md) and
[Backup and Restore](backup-restore.md).
...
- **A healthy container does not mean the schema migration worked.** The
  ...
  [Update](update.md#reading-the-migration-output).

## Troubleshooting

Symptom, checks, action: [Troubleshooting](troubleshooting.md).

Reference documents: [Configuration](configuration.md) for every
setting, and [Deployment Options](deployment-options.md) for using your
