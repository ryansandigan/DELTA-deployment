...
See
[Install Docker on Windows](docker/windows.md#1-turn-on-the-windows-features-wsl-2-needs).
...
=== "PowerShell"
    ...
    ```powershell
    docker compose config | findstr image
    ```
...
secret without one -
[Configuration](configuration.md#generating-secrets).
...
- `database files are incompatible with server` - the `pgdata` volume was
  ...
  back. See [Update](update.md#do-not-upgrade-postgresql-at-the-same-time).
...
| `permission denied to create extension "postgis"` | External database without PostGIS pre-created | A superuser must run `CREATE EXTENSION postgis;` and `CREATE EXTENSION pgcrypto;` first - [Deployment Options](deployment-options.md#what-the-external-database-must-provide) |
...
Full detail: [Update](update.md#reading-the-migration-output).
...
See [Configuration](configuration.md#required-settings).
...
- Enable HTTPS - [step 3 of the installation guide](index.md#3-set-up-https)
...
See [Configuration](configuration.md#project-identity-and-why-it-matters).
...
this - see
[Configuration](configuration.md#volumes-and-persistence).
