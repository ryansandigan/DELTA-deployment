...
- [Connection strings and special characters](#connection-strings-and-special-characters)
- [Generating secrets](#generating-secrets)
- [Email and SMTP](#email-and-smtp)
- [Authentication](#authentication)
- [Ports and networking](#ports-and-networking)
- [Volumes and persistence](#volumes-and-persistence)
- [Project identity and why it matters](#project-identity-and-why-it-matters)
- [Settings deliberately not included](#settings-deliberately-not-included)
...
**It must be `https://` for any real hostname.** DELTA marks session cookies
`Secure`, so a browser will not return them over plain HTTP. Browsers make an
exception for `http://localhost`, which is why local testing over HTTP works and
the same deployment on a real hostname silently fails to keep anyone signed in.
...
[deployment-options.md](deployment-options.md#b---external-database) walks
through it.
...
> [backup-restore.md](backup-restore.md#what-a-backup-contains).
...
[step 6.2 of the installation guide](../README.md#62-replace-the-built-in-administrator-password)
walks through.
...
[deployment-options.md](deployment-options.md#c---existing-proxy-load-balancer-or-waf)
...
Backing these up is [backup-restore.md](backup-restore.md). The one thing to
