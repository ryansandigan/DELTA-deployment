...
- [Pin the image](#pin-the-image)
- [The update procedure](#the-update-procedure)
- [Reading the migration output](#reading-the-migration-output)
- [If the update fails](#if-the-update-fails)
- [Do not upgrade PostgreSQL at the same time](#do-not-upgrade-postgresql-at-the-same-time)
...
Follow [backup-restore.md](backup-restore.md) and take a full backup: the
...
> **If the backup fails, stop. Do not update.** You would be starting an
> irreversible change with no way to undo it.
...
External database (patterns B and D):
...
External database (patterns B and D), where there is no `db` container:
