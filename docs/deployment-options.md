...
[installation guide](../README.md#install-delta). Make these edits **before**
...
[installation guide](../README.md#install-delta).
...
[backup-restore.md](backup-restore.md#taking-a-backup). A database backup on
...
> **Removing the bundled nginx does not make the deployment private.** Your
> gateway is the thing that exposes it, and it is outside this repository's
> control. During the deployment procedure you must **not** enable the route on
> your gateway until step 7.
>
> Steps 5 and 6 of the [installation guide](../README.md#install-delta) start DELTA,
> verify the database, and
> change the publicly-known administrator password. Publishing on `127.0.0.1` or
> a private address is what keeps that window closed. Enable the public route
> afterwards.
...
> HTTPS. See [troubleshooting.md](troubleshooting.md#https-and-sessions).
...
- **The comparison must stay a whole-string equality test.** A regex, prefix,
  suffix or wildcard match turns a scheme correction into an origin bypass, and
  `https://delta.example.org.attacker.example` starts being accepted. Only the
  scheme is ever rewritten, never the host. A foreign origin must reach DELTA
  untouched so DELTA still rejects it, and a request with no `Origin` must
  arrive with none.
...
> [configuration.md](configuration.md#project-identity-and-why-it-matters).
...
[backup-restore.md](backup-restore.md#consistency-between-database-and-uploads).
